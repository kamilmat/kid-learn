...
// Moduły i ekrany rodzica ładowane leniwie: same dane 2000 czytanek to ~40%
// bundla, a dziecko wchodzące w Literki nie ma powodu ich pobierać. Chunki
// siedzą w precache service workera, więc offline działa jak wcześniej.
const LettersModule = lazy(() => import('@/modules/letters').then((m) => ({ default: m.LettersModule })))
const ReadingModule = lazy(() => import('@/modules/reading').then((m) => ({ default: m.ReadingModule })))
const NumbersModule = lazy(() => import('@/modules/numbers').then((m) => ({ default: m.NumbersModule })))
const CzytankiModule = lazy(() => import('@/modules/czytanki').then((m) => ({ default: m.CzytankiModule })))
const SettingsScreen = lazy(() => import('@/shared/settings/components').then((m) => ({ default: m.SettingsScreen })))
const ReportScreen = lazy(() =>
  import('@/shared/stats/components/ReportScreen').then((m) => ({ default: m.ReportScreen })),
)
      ...
      >
        <ErrorBoundary>
          {/* Pusty fallback: chunk leci z cache w kilka ms, a spinner zdążyłby
              tylko mrugnąć. Ekrany dziecka i tak mówią audio, co się dzieje. */}
