...
- **Moduł 4:** czytanki — 2000 krótkich tekstów (4 grupy trudności po 500: 100 ręcznych + 1900 z puli generowanej), tap sylaby → audio, długi tap → całe słowo, ▶ czyta całość, ❓ mini-pytanie o rozumienie, A|B tryb przypominajki literek, 🎲 tryb losowy (talia bez powtórek)
...
├── modules/czytanki/      # moduł 4 — czytanki: tap sylaby → audio, long-press → słowo, ▶ czyta całość; 2000 czytanek, 4 grupy po 500; lista stronicowana + 🎲 losowo
...
│   ├── data/               # czytanki.ts (`CZYTANKI_LEGACY` cz-01…cz-100 ręczne + pula generowana, `CZYTANKI` uporządkowane grupami), types, audioKeys (slugPl → cz-syl-*/cz-word-*, questionAudioKeys → cz-q-NN albo słowa pytania, letterUnitAudioKey → letter-*/cz-let-*)
│   │                       # generated/g*.ts (1900 czytanek w zwartym formacie `compact.ts`, cz-101…cz-2000) + generated/lex-*.ts; lexicon.ts/lexiconText.ts (słownik = budżet nagrań)
│   │                       # validate.ts (reguły grup/pytań) + `scripts/czytanki-validate.ts`; layoutScene.ts (sceny z listy emoji); deck.ts + deckWeight.ts (talia 🎲)
...
│   ├── store/              # czytankiStore (Zustand + persist) — persist key `iskierki-czytanki-v1`, version 4 (`readCounts`, `lastCountedAt`, `answeredQuestionIds`, `comprehensionResults`, `decks`)
...
│   ├── settings/          # store + math gate + UI; persist key `iskierki-state-v1` (persist version: 8)
│   │                      # settings: humorMode + reading.wordAnimations + reading.wildCelebrationFreq + questionsPerSession + secondAttempt + letters.promptMode(+ByLevel) + czytanki.{echoMode,tempo,mergedSyllables,spellMode,randomMode}
...
- **Persist kilka storage** (stan po Fali 2): `iskierki-state-v1` (settings + math gate + humorMode + reading.* + `questionsPerSession` + `secondAttempt` + `letters.promptMode(+ByLevel)` + `czytanki.{echoMode,tempo,mergedSyllables,spellMode,randomMode}`; klucz `name` to `iskierki-state-v1`, **`version: 8`** — nie mylić jednego z drugim; `migrate` v4→v5 mapuje `sessionLength`→`questionsPerSession`, v5→v6 mapuje `promptMode: 'both'`→`'phoneme'`, v6→v7 dokłada `czytanki.spellMode`, v7→v8 `czytanki.randomMode`), `iskierki-letters-v1` (moduł 1, **`version: 2`**: `dailyLetter`, `dailyDoneDayKey`), `iskierki-reading-v1` (moduł 2 progres, `version: 1`), `iskierki-numbers-v1` (moduł 3, **`version: 3`**: `factsCorrect`, `recentOutcomes`) i `iskierki-czytanki-v1` (moduł 4 — openedIds, seenIntros, `wordTaps`, `timeMs`, **`version: 4`**: `readCounts`, `lastCountedAt`, `answeredQuestionIds`, `comprehensionResults` — bez bumpu, default w `merge`; v4 `decks` = talie trybu 🎲). Reset jednego nie kasuje pozostałych.
...
pnpm test --run       # testy (1091/1091 zielone; CI odpala je PRZED buildem). `vitest.config.ts` wyklucza `**/.claude/**` — bez tego zbiera testy ze starych worktree'ów agentów
pnpm audio:czytanki   # generuj czytanki-syllables.json (752) + czytanki-words.json (1374) + czytanki-questions.json (99, tylko cz-01…cz-100) z data/czytanki.ts (moduł 4)
npx tsx scripts/czytanki-validate.ts < /dev/null  # pula generowana: reguły grup, słownik, duplikaty, pytania, BRAKI NAGRAŃ (nie jest w CI — tam pilnuje tego `czytanki.test.ts`)
...
- **`CzytankaScene` liczy pozycje aktorów w PIKSELACH z realnej wysokości sceny**, nie z procentów w danych: mapuje zakres `y` sceny na pas między górną krawędzią a rzędem przycisków (który leży NA scenie, 80 px). Procenty w danych niosą więc tylko RANKING (kto wyżej), nie absolutną pozycję. Dlatego zmiana `layoutScene` nie wymaga przeliczania 100 ręcznych scen (mają `y` 15–85) — ale i dlatego scena, w której wszyscy aktorzy mają prawie równe `y`, dostaje osobną gałąź (`MIN_SPREAD`), żeby szum 1 punktu nie zamienił się w 200 px głębi.
- **Pula generowana czytanek = TYLKO formy ze słownika** (`data/lexicon.ts` ← `lexiconText.ts` + `generated/lex-*.ts`). Każda forma to jedno nagranie `cz-word-*` i JEDEN podział na sylaby (konflikt podziału = błąd testu). Nową czytankę dopisujesz do `generated/g<grupa>*.ts` w formacie `compact.ts`, a potem `npx tsx scripts/czytanki-validate.ts < /dev/null` (reguły grup, słownik, duplikaty, pytanie) → `pnpm audio:build`. Pytania ❓ puli NIE mają klipu `cz-q-*` — grają słowa pytania po kolei (`questionWords`), więc słowa pytania też muszą być w słowniku. Sceny puli liczy `layoutScene` z listy emoji (deterministycznie, seed = numer); kafelek = pierwszy przedmiot/zwierzę sceny, nie poprawna odpowiedź.
- **Talia 🎲 (`deck.ts`) jest per grupa w `czytankiStore.decks`** — `drawNext` pogadza zapisaną talię z pulą (`reconcileDeck`: znikające id wypadają, nowe wchodzą do nieprzeczytanej części), więc dokładanie czytanek NIE wymaga migracji. W widoku z `?los=<grupa>` scena jest zakryta do przeczytania (`revealSceneAfterRead`), ◀ ukryte, ▶ losuje dalej.
