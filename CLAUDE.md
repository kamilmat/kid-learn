...
│   ├── store/              # czytankiStore (Zustand + persist) — persist key `iskierki-czytanki-v1`, version 3 (`readCounts`, `lastCountedAt`, `answeredQuestionIds`)
...
pnpm test --run       # testy (1045/1045 zielone: 926 src + 119 scripts, po Fali 2). `vitest.config.ts` wyklucza `**/.claude/**` — bez tego zbiera testy ze starych worktree'ów agentów
